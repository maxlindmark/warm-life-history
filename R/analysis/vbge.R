...
# UPDATE:
# I think my priors are too narrow. I need to look into 1) rescaling 2) tigher priors
# on the random sd's
# https://discourse.mc-stan.org/t/brms-multilevel-nonlinear-model-model-fails-to-converge-when-converted-from-individual-level-to-multilevel/4691
# https://discourse.mc-stan.org/t/non-linear-model-fitting-problem/9038
# https://rdrr.io/cran/brms/man/set_prior.html
# The latter one can be used to check to see e.g. uniform priors... so that I can't
# get negative values


# Prior predictive check: BT+FM ====================================================
M0fmbt <- brm(
  bf(length_cm ~ Linf*(1-exp(-K*(age-t0))),
     Linf ~ 1, t0 ~ 1, K ~ 1, nl = TRUE),
  data = dfm, family = gaussian(),
  prior = c(prior(normal(40, 4), nlpar = "Linf"),
            prior(normal(-0.5, 0.4), nlpar = "t0"),
            prior(normal(0.17, 0.05), nlpar = "K")),
  sample_prior = "only", 
  iter = 1000, thin = 1, cores = 1, chains = 1, seed = 1)

plot(conditional_effects(M0fmbt), points = TRUE)
ggsave("figures/vbge/vbge_prior_pred_check.png", width = 6.5, height = 6.5, dpi = 600)

pp_check(M0fmbt, nsamples = 50)
ggsave("figures/vbge/vbge_prior_pred_check2.png", width = 6.5, height = 6.5, dpi = 600)
...
prior <-
  ...
  prior(normal(0.17, 0.05), nlpar = "KFM") +
  prior(normal(0.17, 0.05), nlpar = "KBT") +
  prior(normal(40, 4), nlpar = "LinfFM") +
  prior(normal(40, 4), nlpar = "LinfBT")
...
m1 <- 
  brm(
    ...
    family = gaussian(),
    prior = prior,
    iter = 1500, thin = 1, cores = 2, chains = 2, inits = "0",
...
# Compare models
loo_compare(waic_m1, waic_m2, waic_m3, waic_m5, waic_m8)

compare_ic(waic_m1, waic_m2, waic_m3, waic_m5, waic_m8)

# Using model 1 for now
...
# sigma_Intercept      0.21      0.02     0.17     0.25 1.00     2759      946
# t0FM_Intercept      -0.43      0.07    -0.56    -0.30 1.00     1489     1240
# t0BT_Intercept      -0.14      0.03    -0.20    -0.08 1.00     1582      987
# KFM_Intercept        0.15      0.01     0.12     0.17 1.00      617      887
# KBT_Intercept        0.19      0.02     0.15     0.23 1.02      288      538
# LinfFM_Intercept    39.50      1.92    35.97    43.47 1.00      742      914
# LinfBT_Intercept    42.66      3.01    36.72    48.81 1.00      440      742
# sigma_age            0.15      0.00     0.14     0.16 1.00     2976      930
...
p1 <- dfm %>% 
  ...
  ggplot(aes(x = factor(age), y = length_cm, color = area, fill = area)) +
    ...
    geom_jitter(data = dfm, alpha = 0.2, width = 0.4,
                height = 0, size = 0.8) +
...
pWord1 <- p1 + theme(text = element_text(size = 20), # 12 for word doc
...
pWord2 <- p2 + theme(text = element_text(size = 20), # 12 for word doc
                     legend.position = c(0.7, 0.1), 
                     legend.title = element_text(size = 20),
                     legend.text = element_text(size = 20))

ggsave("figures/vbge/vbge_pred_year.png", width = 6.5, height = 6.5, dpi = 600)
...
ggsave("figures/vbge/vbge_ppc.png", width = 6.5, height = 6.5, dpi = 600)
